#!/bin/bash
#SBATCH --job-name=miniprot_FPRA
#SBATCH --output=runs/miniprot_FPRA_%j.out
#SBATCH --error=runs/miniprot_FPRA_%j.err
...
GENOME="/mnt/project/FjellheimLab/EGAPx/Lolium_perenne/genome.fna"
PROTEINS="/mnt/project/FjellheimLab/martpali/AnnualPerennial/nf-denovoslim/FPRA/proteins/FPRA.faa"
SPECIES="FPRA"

echo "=== $SPECIES proteins vs Lolium perenne Kyuss2.0 genome ==="
echo "Started: $(date)"
$MINIPROT -t ${SLURM_CPUS_PER_TASK} --gff "$GENOME" "$PROTEINS" > "$OUTDIR/${SPECIES}_vs_Kyuss2.gff" 2> "$OUTDIR/${SPECIES}_vs_Kyuss2.log"
echo "Finished miniprot: $(date)"

echo ""
echo "=== Analyzing hit rates by protein length ==="

GFF="$OUTDIR/${SPECIES}_vs_Kyuss2.gff"
