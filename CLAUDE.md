...
13. **Locus clustering is optional** — only runs when `--reference_genome` is provided. Uses minimap2 `-x splice` to map representatives to the genome, then groups overlapping alignments into loci and picks one transcript per locus. Unmapped transcripts are retained. Filters on **mapq** (not nucleotide identity) because cross-genus divergence makes nt identity unreliable — `--locus_min_mapq 5` (default) accepts alignments with a clear best location. The step sits between MMSEQS2_CLUSTER and DIAMOND_BLASTX.
