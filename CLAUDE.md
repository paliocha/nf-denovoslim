...
13. **Locus clustering is optional** — only runs when `--reference_genome` is provided. Uses minimap2 `-x splice` to map representatives to the genome. Two modes: **gene-level** (when `--reference_gff` also provided) assigns each transcript to the best-overlapping annotated gene and picks one per gene; **coordinate-overlap** (no GFF) merges nearby alignments into ad-hoc loci. Unmapped transcripts are always retained. Filters on **mapq** (not nucleotide identity) because cross-genus divergence makes nt identity unreliable — `--locus_min_mapq 5` (default) accepts alignments with a clear best location. The step sits between MMSEQS2_CLUSTER and DIAMOND_BLASTX.
