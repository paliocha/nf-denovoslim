...
| **Local work** | `$TMPDIR` | Per-job directory on compute nodes; `/work/users/$USER` on login nodes — node-local XFS | **No** | Auto-cleaned on job end (compute); persistent on login | None |

### $TMPDIR — node-local SSD

**On compute nodes:** `$TMPDIR` is a **per-job directory** created by SLURM and automatically cleaned when the job finishes. Do NOT use `$TMPDIR/$USER` or `/work/users/$USER` — only `$TMPDIR` itself is guaranteed to exist and be cleaned.

**On login nodes:** `$TMPDIR` is still `/work/users/$USER` (persistent, not auto-cleaned).

Each node has its own physical `/work` disk — files on the login node's `/work` are NOT visible from compute nodes and vice versa.

**Key facts:**
- **Per-job on compute nodes** — SLURM creates and destroys the directory automatically. Files left outside `$TMPDIR` may be removed without notice.
...
**Cleanup of stale Nextflow scratch dirs (login node only):**
```bash
rm -rf /work/users/$USER/nxf-*
```

Compute nodes no longer need manual cleanup — `$TMPDIR` is auto-cleaned per job.
...
- `scratch = '$TMPDIR'` — all tasks run on node-local SSD (per-job `$TMPDIR` on compute nodes); Nextflow creates per-task `nxf-XXXXXX` dirs and rsync's outputs back to NFS work dir
